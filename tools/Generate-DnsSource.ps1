...
Write-Utf8Lf (Join-Path $projectionRoot 'CmdletModel.json') ($cmdlets | ConvertTo-Json -Depth 100)
...
'@
Write-Utf8Lf (Join-Path $modelOutputRoot 'CfDnsRecordModels.cs') $models
...
"@
Write-Utf8Lf (Join-Path $metadataOutputRoot 'CfDnsRecordOperationMetadata.cs') $operationsSource
...
"@
    Write-Utf8Lf (Join-Path $metadataOutputRoot "$className.cs") $source
}

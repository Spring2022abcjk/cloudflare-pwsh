...
static Dictionary<string, NormalizedSchema> CollectSchemas(NormalizedDocument source, NormalizedOperation operation)
{
    ...
    while (pending.Count > 0)
    {
        ...
        foreach (var child in schema.Properties.Values.Select(x => x.Schema).Concat(schema.OneOf).Concat(schema.AnyOf).Concat(schema.AllOf).Append(schema.Items ?? string.Empty).Append(schema.AdditionalPropertiesSchema ?? string.Empty).Where(x => !string.IsNullOrEmpty(x)).OrderBy(x => x, StringComparer.Ordinal))

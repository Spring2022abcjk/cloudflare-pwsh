...
function ConvertTo-CSharpStringArray {
    ...
    $items = @($Values | ForEach-Object { ConvertTo-CSharpString $_ })
    if ($items.Count -eq 0) { return '[]' }
    return "[$($items -join ', ')]"
}

function ConvertTo-CSharpParameterMetadata {
    param([object]$Parameter)
    return "new GeneratedParameterMetadata { Name = $(ConvertTo-CSharpString $Parameter.name), Location = $(ConvertTo-CSharpString $Parameter.location), Required = $(ConvertTo-CSharpBool $Parameter.required) }"
}

function ConvertTo-CSharpRequestRepresentationMetadata {
    param([object]$Representation)
    $parts = @()
    if ($null -ne (Get-JsonPropertyValue $Representation 'parts')) {
        $parts = @($Representation.parts | ForEach-Object {
    ...
    $partsLiteral = if ($parts.Count -eq 0) { '[]' } else { "[$($parts -join ', ')]" }
    return "new GeneratedRequestRepresentationMetadata { ContentType = $(ConvertTo-CSharpString $Representation.contentType), BodyParameterName = $(ConvertTo-CSharpString 'body'), Parts = $partsLiteral }"
}

function ConvertTo-CSharpResponseRepresentationMetadata {
    param([object]$ResponseCase, [object]$Representation)
    $statusCode = $null
    if ($ResponseCase.statusSelector.kind -eq 'Exact') {
        $parsedStatus = 0
        if ([int]::TryParse([string]$ResponseCase.statusSelector.value, [ref]$parsedStatus)) { $statusCode = $parsedStatus }
    }
    return "new GeneratedResponseRepresentationMetadata { StatusCode = $(if ($null -eq $statusCode) { 'null' } else { [string]$statusCode }), ContentType = $(ConvertTo-CSharpString $Representation.contentType), EnvelopePolicy = $(ConvertTo-CSharpString $Representation.envelopePolicy), ParsingMode = $(ConvertTo-CSharpString $Representation.parsingMode) }"
}
...
'@
Write-Utf8CrLf (Join-Path $modelOutputRoot 'CfDnsRecordModels.cs') ($models + "`r`n")
...
$(($operations | Sort-Object { $_.Operation.operationId } | ForEach-Object {
    ...
    $requests = @()
    $requestBody = Get-JsonPropertyValue $op 'requestBody'
    $effectivePresence = if ($null -ne $requestBody) { Get-JsonPropertyValue $requestBody 'effectivePresence' } else { $null }
    $presence = if ($null -ne $requestBody) { Get-JsonPropertyValue $requestBody 'presence' } else { $null }
    $bodyIsPresent = $null -ne $requestBody -and $effectivePresence -ne 'absent' -and $presence -ne 'declared-but-observed-absent'
    if ($bodyIsPresent) { $requests = @($op.requestBody.representations | ForEach-Object { ConvertTo-CSharpRequestRepresentationMetadata $_ }) }
    $requestLiteral = if ($requests.Count -eq 0) { '[]' } else { "[$($requests -join ', ')]" }
    $responses = @($op.responses | ForEach-Object { $case = $_; $_.representations | ForEach-Object { ConvertTo-CSharpResponseRepresentationMetadata $case $_ } })
    $responseLiteral = if ($responses.Count -eq 0) { '[]' } else { "[$($responses -join ', ')]" }
    $pagination = ConvertTo-CSharpPaginationMetadata (Get-JsonPropertyValue $op 'pagination')
    @"
        new GeneratedOperationMetadata
        {
            OperationId = $(ConvertTo-CSharpString $op.operationId),
            Method = $(ConvertTo-CSharpString $op.method),
            PathTemplate = $(ConvertTo-CSharpString $op.pathTemplate),
            Parameters = $parameterLiteral,
            RequestRepresentations = $requestLiteral,
            ResponseRepresentations = $responseLiteral,
...
"@
Write-Utf8CrLf (Join-Path $metadataOutputRoot 'CfDnsRecordRuntimeMetadata.cs') $runtimeOperationsSource
...
[pscustomobject]@{
    ...
    GeneratedFiles = @('Generated/Models/CfDnsRecordModels.cs', 'Generated/Metadata/CfDnsRecordOperationMetadata.cs', 'Generated/Metadata/CfDnsRecordRuntimeMetadata.cs')
